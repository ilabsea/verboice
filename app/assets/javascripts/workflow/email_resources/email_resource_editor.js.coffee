#= require workflow/email_resources/email_resource

onWorkflow ->
  class window.EmailResourceEditor
    ...
    constructor: (parent, hash = {}) ->
      ...
      if hash.guid?
        EmailResource.find hash.guid, (result) =>
          @resource(result)
    ...
    get_resources: (query, source) =>
      EmailResource.search query, (results) =>
        source(results)
    ...
    next: =>
      switch @type()
        when 'new'
          @resource(new EmailResource(name: @name()))
        when 'existing'
          EmailResource.find @existing_resource_guid(), (result) =>
            @resource(result)
