...
-export([time_difference_in_seconds/2, strftime/1]).
...
strftime(DateTime) ->
  {datetime, {{Y, Month, D}, {H, Minute, S}}} = DateTime,
  Date = to_string(Y) ++ "/" ++ to_string(Month) ++ "/" ++ to_string(D),
  Time = to_string(H) ++ ":" ++ to_string(Minute) ++ ":" ++ to_string(S),
  Date ++ " " ++ Time.
