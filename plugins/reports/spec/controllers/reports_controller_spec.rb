...
describe ReportsController, :type => :controller  do
  ...
  it "should create report after call finished" do
    get :call_finished, CallSid: call_log.id, "CallStatus"=>"Missing", "From"=>"verboice_13", "CallDuration"=>"10"
    response.body.should eq("")
  end

  it "should create report after call finished" do
    expect {
      get :call_finished, CallSid: call_log.id, "CallStatus"=>"completed", "From"=>"verboice_13", "CallDuration"=>"10"
  ...
  context 'when process upload' do
    ...
    it "should return sucess" do 
    	get :upload_excel, :project_id => project.id
    	assert_response :success
    end

    it "should read file for import" do
      post :upload_excel, :project_id => project.id, :file => file 
      file_name = Rails.root.join('public', 'docs', file.original_filename)
      file = mock('file')
      File.should_receive(:open).with(file_name, "w")
      file.should_receive(:write).with("text")
      # File.exist?().should eq(true)
    end
